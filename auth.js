...
function login() {
    ...
    firebase.auth().signInWithEmailAndPassword(email, password)
        .then((userCredential) => {
            ...
            // Teacher login — checked against the shared allowlist in teacher-config.js
...
firebase.auth().onAuthStateChanged((user) => {
    ...
    // If logged in → still block unauthorized access
        ...
        // Prevent anyone except an allowlisted teacher from opening edit.html
        if (path.includes("edit.html") && !isTeacherEmail(user.email)) {
